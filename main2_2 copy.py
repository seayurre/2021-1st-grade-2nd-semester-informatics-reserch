...
f = open("C:/Users/yulyu/Desktop/과제연구 집탐 코드/20210901 도로_가공.txt", 'r',encoding="cp949")
lines = f.readlines()
...
for x in range(1, 1633):
    h[x]=float(HR1.휴리스틱1(value_to_key(x),종점명,p)) #1~1632

'''다른 점들까지의 g는 모두 무한으로 설정해놓고 시작점만 0으로 설정해두기'''
g[시점명_index]=0
f[시점명_index]=float(h[시점명_index])
print(f[시점명_index])
...
while not pq.empty():
    ...
    # chk[uindex]=True
    if uindex in graph.keys():
        for node in graph[uindex]: #graph[uindex].items()->uindex와 연결된 모든 점의 [index, 가중치]값들
            thisindex=node[0]#인덱스
            thisvalue=float(node[1])#가중치

            if float(g[uindex]+thisvalue)<g[thisindex]:
                g[thisindex]=float(g[uindex]+thisvalue)
                prev[thisindex]=uindex
                f[thisindex]=float(g[thisindex]+h[thisindex])
                pq.put([(-1)*f[thisindex],thisindex])
    else:
        pass
        #print("graph에 {}번:{} 점이 없다! 큰일이네!".format(uindex, value_to_key(uindex)))
...
printlist_반전 = printlist[::-1]

for i in printlist_반전:
    print(i, value_to_key(i),"g:",g[i],"h:",h[i],"f:",f[i])
print("%d %s g: %f h: %f f: %f"%(종점명_index,value_to_key(종점명_index),g[종점명_index],h[종점명_index],f[종점명_index]))


print("시간 %f! 따라란~"%g[종점명_index])
